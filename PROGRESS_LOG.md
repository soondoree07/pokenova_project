...
마지막 업데이트: 2026-03-31 (엔트리 구성 페이지 신규 추가 및 대규모 개선)
...
| 기술 설명 (Z기술 18개) | ⏳ 미번역 |
| 기술 설명 (Gen8-9 24개, ID 827~850) | ✅ 완료 |
...
### 엔트리 구성 페이지 (entry.html) 신규 추가

- [x] `entry.html` 신규 생성: 메인 엔트리 6슬롯 + 벤치 엔트리 6슬롯 팀 빌더 페이지
- [x] 포켓몬 선택 모달: 3열/이미지 88px/도감 순서(ID순) 정렬, 타입 필터 지원
- [x] 포켓몬 선택 후 특성 선택 모달 자동 표시 (특성 1개면 자동 적용)
- [x] 슬롯에 특성 칩 추가 (클릭 시 변경 가능, 숨겨진 특성 주황색 표시)
- [x] 포켓몬 선택 후 도구 선택 모달 연속 표시 (카테고리 그룹 필터 + 이름 검색)
- [x] localStorage로 저장/불러오기
- [x] `index.html` nav에 엔트리 구성 버튼 추가

### 커밋 목록

| 커밋 | 내용 |
|------|------|
| 9e53d9d | 엔트리 구성 페이지 추가 (entry.html) |
| d5df2bd | entry.html 대규모 개선 |
...
- [x] **모달 이름 표시 개선**: 모든 카드(원종+폼) 상단에 포켓몬 이름 표시, 폼이름은 이름 아래 `(폼이름)` 형태로 표시
- [x] **특성 클릭 → 특성 보유 포켓몬 목록 모달**: 특성 클릭 시 기존 모달 닫히고 특성 상세 모달(설명 + 보유 포켓몬 그리드) 표시, 포켓몬 클릭 시 해당 포켓몬 모달로 이동
- [x] **특성 hover 스타일**: 커서 포인터 + 빨간 테두리

### 포켓몬 데이터 추가/수정

- [x] **약어리**: `단독의 모습` formLabel 추가
- [x] **에써르**: `수컷` formLabel + 암컷 폼 추가 (ID 10186)
- [x] **다투곰**: `히스이의 모습` formLabel + `붉은달의 모습` alt 폼 추가 (ID 10272, 특성: 심안)
- [x] **스트린더**: `하이한 모습` formLabel + `로우한 모습` alt 폼 추가 (ID 10184); 거다이맥스 2개 → 1개로 통합
- [x] **싸리용**: `젖힌 모습` formLabel + `늘어진 모습`(10258) / `뻗은 모습`(10259) alt 폼 추가
- [x] **마기아나**: 두 번째 mega 항목 → `500년전의 모습` alt 폼으로 수정 (ID 10147)
- [x] **패러독스 뱃지**: 1005~1010, 1020~1023에 `is_paradox: true` 추가 (고동치는달, 무쇠무인, 코라이돈, 미라이돈, 굽이치는물결, 무쇠잎새, 꿰뚫는화염, 날뛰는우레, 무쇠암석, 무쇠감투)

### 특성 데이터 수정

- [x] **특성 이름 수정**: 예리함·재앙의목간·재앙의그릇·재앙의구슬 (공백 추가), 황금몸·테라셸·제로포밍 설명 수정
- [x] **특성 이름 오류 수정**: 뉴트론포스→브레인포스, 하트파워→치유의마음, 테라쉘→테라셸, 테라폼제로→제로포밍
- [x] **영어 특성명 60개 한국어 일괄 변환** (quiz_pokemon.json 내 abilities ko 필드)
- [x] **심안(마음의눈)→심안**, **물흡수→저수** 수정

### 기술 데이터

- [x] **거다이맥스 전용 기술 34개 추가** (ID 851~884): 거다이편달~거다이감쇠, 거다이일격, 거다이연격, 무한다이빔
- [x] **도감 내 거다이맥스 기술명 전부 수정** (docx 기준): 거다이넝쿨매질→편달, 야화→옥염, 연격포→포격 등 30개 이름 교체

### 메가진화 추가 (레전드 Z-A)

- [x] **12종 Z-A 메가 스프라이트**: 포켓몬 위키(pokemon.fandom.com) 이미지 사용
  - 메가한카리아스Z, 메가라이츄X, 메가라이츄Y, 메가앱솔Z, 메가찌르호크
  - 메가루카리오Z, 메가히드런, 메가다크라이, 메가골루그, 메가냐오닉스, 메가제라오라, 메가스코빌런
- [x] **CANONICAL_MEGA** 목록에 12종 추가 (pokedex.html + index.html)
- [x] **메가마기아나/메가싸리용** 특성 `?` 표시 수정

### 오류 수정

- [x] 사이트 내용 전체 검수: 스프라이트 오류, 특성 이름 불일치, 중복/고아 항목 등 발견·수정
- [x] 도감 영어 특성명 60개 → 한국어로 일괄 수정

### 커밋 목록

| 커밋 | 내용 |
|------|------|
| 6e9714c | 패러독스/UB 뱃지, 리전 진화체, Gen8-9 폼 대거 추가 |
| febfdfa | 약어리 단독의 모습, 에써르 수컷/암컷 구분 |
| 9387ed5 | 다투곰 붉은달의 모습 + 히스이의 모습 label |
| c6401a8 | 패러독스 is_paradox 1005-1010, 1020-1023 |
| 075c270 | 스트린더 하이한/로우한 모습, gmax 1개로 통합 |
| 70cce34 | 특성 이름/설명 수정 (예리함, 재앙의 목간 등) |
| 1f9496d | 영어 특성명 60개 한국어 변환 + 모달 이름/폼라벨 개선 |
| e486858 | 특성 이름 불일치 4건 수정 |
| 5fb2188 | 한카리아스Z/마기아나/싸리용 스프라이트 수정 |
| 8a7322e | 싸리용 세 가지 모습 추가 |
| 84a7b29 | 거다이맥스 전용 기술 34개 추가 (ID 851-884) |
| 9693c42 | 특성 클릭 → 보유 포켓몬 모달 기능 추가 |
| 727e85d | CANONICAL_MEGA 화이트리스트 보강 |
| 54ceacf | 도감 거다이맥스 기술명 전부 수정 (docx 기준) |
| c5d6c0b | 메가마기아나/메가싸리용 특성 ? 표시 수정 |
| 79fb2bd | Z-A 메가 12종 스프라이트 추가 (위키 이미지) |
| e041468 | 특성 모달 열 때 포켓몬 모달 먼저 닫기 |
